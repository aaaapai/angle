...
public:
    ...
    struct templated_iterator
    {
        ...
        templated_iterator & operator++()
        {
            ...
            // The loop always stops because the special end entry has has_value() == true.
            return *this;
        }
    ...
    iterator find(const FindKey & key)
    {
        ...
        {
            if (compares_equal(key, it->value)) [[likely]]
                return { it };
        }
        return end();
    }
...
private:
    ...
    {
        int8_t desired = static_cast<int8_t>(std::bit_width(num_buckets) - 1);
    ...
    {
        using std::swap;
        if (num_slots_minus_one == 0 || distance_from_desired == max_lookups || num_elements + 1 > (num_slots_minus_one + 1) * static_cast<double>(_max_load_factor)) [[unlikely]]
        {
            grow();
        ...
        for (++distance_from_desired, ++current_entry;; ++current_entry)
        {
            ...
            {
                ++distance_from_desired;
                if (distance_from_desired == max_lookups) [[unlikely]]
                {
...
struct prime_number_hash_policy
{
    ...
    mod_function next_size_over(size_t & size) const
    {
        ...
        // uint64_t rather than size_t, so that this header can be included on
        // a 32-bit target.
        //
        // Two thirds of this list is above 2^32, and a braced initialiser is
        // list-initialisation, where narrowing is an error rather than a
        // warning. The declaration is inside a plain struct, so it is checked
        // wherever the header is parsed -- a 32-bit build could not include
        // flat_hash_map.hpp at all, whether or not this policy was ever named.
        // Widening the element type changes nothing where size_t is already 64
        // bits, and on 32-bit the entries past 2^32 were unreachable regardless:
        // such a process cannot hold that many buckets.
        ...
        const uint64_t * found = std::lower_bound(std::begin(prime_list), std::end(prime_list) - 1, static_cast<uint64_t>(size));
        size = *found;
...
struct fibonacci_hash_policy
{
    ...
    int8_t next_size_over(size_t & size) const
    {
        size = std::max(size_t(2), std::bit_ceil(size));
